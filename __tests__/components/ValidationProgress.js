...
describe('ValidationProgress', () => {
  ...
  it('gets expected results from getText', () => {
    ...
    expect(getText({code: 7}).props.children[0].props.children).toBe('Verifying edits...')
    expect(getText({code: 9}).props.children[0].props.children).toBe('Edit verification complete')
  })
